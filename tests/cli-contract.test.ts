...
import { makeBadSvg, makeGoodSvg } from "../src/fixtures";
...
const workDir = mkdtempSync(join(tmpdir(), "iconlens-contract-"));
const goodPath = join(workDir, "good.svg");
const badPath = join(workDir, "bad.svg");
writeFileSync(goodPath, makeGoodSvg());
writeFileSync(badPath, makeBadSvg());
...
describe("iconlens CLI contract", () => {
  ...
  test("a missing input file exits 3 (I/O)", () => {
    const res = runCli([join(workDir, "missing.svg")]);
    expect(res.code).toBe(3);
